...
switch($view_type) {
    case '':
        $this->assign('title', '現在のルーチン');
        $header = '現在の';
        break;
    case 'forward':
        $this->assign('title', '将来のルーチン');
        $header = '将来の';
        break;
    case 'done':
        $this->assign('title', '過去のルーチン');
        $header = '過去の';
        break;
}
?>
    ...
    <?php echo $this->Form->create('Task', ['type' => 'post']); ?>
        <table id="task-table" class="uk-table p-task-table p-task-table--small-text p-task-table--list">
            <tr>
                ...
                <th class="uk-width-3-6"><?php echo $this->Paginator->sort('title', 'タスク名<i class="fa fa-sort" aria-hidden="true"></i>', array('escape' => false)) ?></th>
            ...
            <?php if(count($tasks_data) === 0): ?>
                <tr><td class="uk-text-center" colspan="4">ルーチンはありません</td></tr>
            <?php else: ?>
                <?php foreach ($tasks_data as $task): ?>
                <tr>
                    <td class="uk-width-1-6 uk-text-center"><?php echo h($task['Task']['begin']); ?></td>
                    <td class="uk-width-3-6"><?php echo $this->Html->link(h($task['Task']['title']), '/tasks/detail/' . h($task['Task']['id'])); ?><span class="c-task-color" style="background-color: <?php echo h($task['Task']['color']); ?>"></span></td>
