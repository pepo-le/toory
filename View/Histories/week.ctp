<?php
$this->assign('title', '履歴週別集計');
$this->Html->addCrumb('履歴', '/histories/');
$this->Html->addCrumb('週別集計');
?>
<div>
    <table class="uk-table p-task-table p-task-table--small-text p-task-table--center">
        ...
        <?php else: ?>
            ...
            <tr>
                ...
                <?php
                    // 日付と週の表示
                    echo date('Y/m', strtotime(h($history['History']['month']))) . '-' .
                    // 対象週 - 月始めの週番号 + 1
                    ( (int)$history['History']['week'] - date('W', strtotime($history['History']['month'])) + 1 ) .
                    '週目';
                ?>
                </td>
